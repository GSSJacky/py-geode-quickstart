...
class GeodeClient:
    '''
    Allows application to store and access data stored in Geode over REST. Please see:
    https://geode.apache.org/docs/guide/114/rest_apps/setup_config.html
    for instructions on setting up Geode's REST service.
    '''
    ...
    def __init__(self, hostname, port, user, password, debug_mode=False):
        ...
        self.base_url = "http://" + hostname + ":" + str(port) + "/geode/v1/"
        if debug_mode:
    ...
    def list_all_regions(self):
        ...
        if data.status_code == 200:
            stringobj="Response from server: "
            stringobj=stringobj.join(data.json())
            logging.debug(stringobj)
            return names
        else:
            self.error_response(data)
    ...
    def adhoc_query(self, query_string):
        ...
        if data.status_code == 200:
            stringobj="Response from server: "
            datastring=self.format_json(data)
            stringobj=stringobj + datastring
            logging.debug(stringobj)
            return datastring
        else:
            self.error_response(data)
    ...
    def format_json(self, jsonobject):
        json_formatted_str = json.dumps(jsonobject.json(), indent=4)
